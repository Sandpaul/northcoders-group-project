...
def test_transform_transaction_returns_correct_created(s3, bucket, example_data):
    file_path = "transaction/2024-02-22/14:50:20.010163.json"
    test_data = read_ingestion_file_data(file_path)
    result = transform_transaction(test_data)
...
@pytest.mark.describe("transform_transaction()")
@pytest.mark.it(
    "should work when passed transaction list with more than one dict"
)  # noqa
def test_transform_transaction_works_on_multiple_dicts(s3, bucket, example_data):
    file_path = "transaction/2024-02-22/14:50:20.010164.json"
    test_data = read_ingestion_file_data(file_path)
    result = transform_transaction(test_data)

    assert result["transaction"].get("transaction_id").get(0) == 9882
    assert result["transaction"].get("transaction_id").get(1) == 9881
    assert result["transaction"].get("purchase_order_id").get(0) is None
